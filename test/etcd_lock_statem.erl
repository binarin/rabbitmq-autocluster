            ...
            ?TRAPEXIT(
               begin
                  ...
                  ?WHENFAIL(ct:pal("Result: ~w~nState:~n~sDynamic:~n~s~s",
                                    [Result, dump_state(State), dump_server_state(DynamicState), dump_history(History)]),
...
dump_history(History) ->
    lists:flatten(lists:zipwith(fun(SeqNo, {State, Result}) ->
                                        io_lib:format("History element ~p and result '~s':~n~s",
                                                      [SeqNo, Result, dump_state(State)])
                                end,
